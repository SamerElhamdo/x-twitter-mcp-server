...
#### الطريقة الأولى: استخدام mcp-proxy (موصى به للـ SSE)
```bash
mcp-proxy --host=0.0.0.0 --port=9000 --allow-origin='*' -- python run_server.py
```

#### الطريقة الثانية: استخدام السكريبت المدمج
```bash
...
#### الطريقة الرابعة: استخدام Python module
```bash
...
#### عند استخدام mcp-proxy (الطريقة الموصى بها):
- **الخادم**: http://localhost:9000
- **التوثيق**: http://localhost:9000/docs
- **المصادقة**: http://localhost:9000/auth
...
## 🚀 استخدام mcp-proxy (موصى به)

### مميزات mcp-proxy:
- ✅ **دعم SSE**: Server-Sent Events يعمل بشكل مثالي
- ✅ **CORS**: معالجة تلقائية لمشاكل Cross-Origin
- ✅ **WebSocket**: دعم كامل للاتصالات في الوقت الفعلي
- ✅ **Proxy**: يعمل كوسيط آمن بين العميل والخادم
- ✅ **منفذ مخصص**: يمكنك تحديد المنفذ الذي تريده (مثل 9000)
